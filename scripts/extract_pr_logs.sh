...
#   ./scripts/extract_pr_logs.sh 329 test         # Only jobs matching "test"
...
if [[ -z "$INPUT" ]]; then
  ...
  echo "  $0 329 test         # Only jobs matching \"test\" from PR #329" >&2
...
suggest_local_command() {
  ...
  case "$job_name" in
    *"vendor"* | *"Vendor"*)
      echo "💡 Reproduce locally: make verify-vendor"
      ;;
    *"test"* | *"Test"*)
    ...
    *"build"* | *"Build"*)
      ...
      ;;
    *"Lint GitHub Actions"* | *"actionlint"* | *"zizmor"*)
      echo "💡 Reproduce locally: go run github.com/rhysd/actionlint/cmd/actionlint@v1.7.10"
      ;;
  esac
}
